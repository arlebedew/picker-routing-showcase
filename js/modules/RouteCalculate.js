...
class RouteCalculate {
	...
	heuristicManhattanDiagonal(a, b) {
		let tieBreaker = 0;

		if (this.enableDotTiebreaker) {

			const 	dx1 = a.x - b.x,
					dy1 = a.y - b.y,
					dx2 = this._depotCoord.x - b.x,
					dy2 = this._depotCoord.y - b.y;
		...
		if (this.allowDiagonals) {
			...
			return straight + diagMult * diag + tieBreaker * 0.001;
		}

		return Math.abs(a.x-b.x) + Math.abs(a.y-b.y) + tieBreaker * 0.001;
	}
	...
	findAStarPath(from, to) {
		...
		const fScoreMap = {
			[startNode.id]: this.heuristicManhattanDiagonal(startNode, goalNode) 
		},
		gScoreMap = { 
			[startNode.id]: 0 
		};
		...
		while (!openSet.isEmpty()) {
			...
			for (const neighbor of neighbors) {
				...
				if (tentativeBetter) {
					...
					fScoreMap[neighbor.id] = tentativeG + this.heuristicManhattanDiagonal(neighbor, goalNode);
	...
	getAisleCellForPick(pick, firstTime) {
		...
		if(isDepot){
			let depotSide = this._warehouse.depotPos;

			if (depotSide === 'top') {
				isDepot = 1;
			}
			else if (depotSide === 'bottom') {
				isDepot = 3;
			}
			else if (depotSide === 'left') {
				isDepot = 4;
			}
			else if (depotSide === 'right') {
				isDepot = 2;
			}
		}
	...
	nearestNeighbor() {
		...
		for (let step = 1; step < this._pickListAndDepo.length; step++) {
			...
			for (let j = 1; j < this._pickListAndDepo.length; j++) {
				if (!visited[j]) {
					const d = this.#nearestNeighborDistance(
	...
	#nearestNeighborDistance(a, b) {
		const dx = a.x - b.x, dy = a.y - b.y;
		return Math.sqrt(dx * dx + dy * dy);
	}

	#totalDistancePerOrder() {
		let totalDistance = 0;

		for (let i = 0; i < this._pickOrder.length - 1; i++) {
			totalDistance += this.#nearestNeighborDistance(
				this._pickOrder[i],
				this._pickOrder[i + 1]
				);
		}
